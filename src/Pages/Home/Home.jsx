...
const Home = () => {
  ...
  let {data, getTitle, getDesc, getName, isLoading} = useQueryData(
    'https://tuwaiq.ezdhaar.com/public/api/home', 'GET', 'Home')
      ...
      <SiteHelmet title={langs[lang].Titles.home}/>
        ...
        <section id="carouselExampleFade" className="carousel slide carousel-fade z-1 custom-padding"  data-bs-ride="carousel" >
          ...
          {allSliders?.map((slide) => (
            <div
              key={slide.id}
              className={`carousel-item ${slide.id === 1 ? "active" : ""} h-100 position-relative d-flex justify-content-center align-items-center`}
              data-bs-slide="fade"
        ...
        {/* About Home Section */}
        <AboutUs  location='about' aboutUs={aboutUs} getName={getName} getDesc={getDesc} />
        ...
        {/* Services */}
        <Service title= {langs[lang].Titles.Services} services={services} getName={getName} getDesc={getDesc} />
        ...
        {/* Blog Home Section */}
        <BlogComp title={langs[lang].Titles.Blog} location='blog' blogs={latestBlogs} getName={getName} getDesc={getDesc}/>
